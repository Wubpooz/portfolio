...
export default function Seo() {
  ...
  useEffect(() => {
    ...
    const projectSlugMatch = new RegExp(/^\/projects\/([^/]+)$/).exec(
      location.pathname,
    );
    const project = projectSlugMatch
      ? getProjectBySlug(locale, projectSlugMatch[1])
      : undefined;
    const isKnownRoute =
      pathname === "/" || isResume || isProjectsIndex || Boolean(project);
    ...
    upsertMeta(
      "og:locale",
      locale === "fr" ? "fr_FR" : locale === "ar" ? "ar_AR" : "en_US",
      true,
    );
