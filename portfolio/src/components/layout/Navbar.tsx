  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-border bg-background/80 backdrop-blur-md">
      <div className="mx-auto flex h-14 max-w-4xl items-center justify-between gap-4 px-6">
        ...
        <Link
          to="/"
          className="font-mono font-bold text-sm tracking-tight text-foreground"
        >
          MW
        </Link>
        ...
        <div className="animate-slide-down border-t border-border bg-background/95 px-4 py-4 shadow-lg backdrop-blur md:hidden">
          ...
          >
            {navLinks.map((link, index) => {
              const isHash = link.href.startsWith("#");
              const className =
                "animate-fade-in rounded-md border border-border bg-card/40 px-3 py-3 text-sm font-mono text-foreground transition-colors hover:bg-muted/30";
              const style = { animationDelay: `${(index * 40).toString()}ms` };
              const onClick = () => {
                setMobileMenuOpen(false);
              };
